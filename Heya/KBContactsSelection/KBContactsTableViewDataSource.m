...
@property (nonatomic, strong) NSMutableArray *selectedContactsRecordIds, * selectedContactsNumbers;
...
- (void)initializeArrays
{
    ...
    _selectedContactsNumbers = [NSMutableArray array];
...
- (void)loadContacts
{
    ...
    [ab loadContacts:^(NSArray *contacts, NSError *error) {
        if (contacts)
        {
            NSMutableArray *tempStorageContact = [NSMutableArray array];
            for (APContact *contact in contacts)
            {
                //If more than one contacts exists then make other contact object with same name but diffrent contact no. & label
                if (contact.phonesWithLabels.count>1)
                {
                    //NSLog(@"FullName: %@",[contact fullName]);
                    for (int j=0; j<contact.phonesWithLabels.count; j++)
                    {
                        APContact *tempContact = [contact copy];
                        
                        NSArray *arrphonesWithLabels = [NSArray arrayWithObjects:[contact.phonesWithLabels objectAtIndex:j], nil];
                        
                        tempContact.phonesWithLabels = [arrphonesWithLabels mutableCopy];
                        
                        [tempStorageContact addObject:tempContact];
                    }
                }
                
                else
                    [tempStorageContact addObject:contact];
                
            }
            
            NSArray *filteredContacts = [self filteredDuplicatedContacts:tempStorageContact];
    ...
    for (APContact *contact in array) {
        ...
        if (_configuration.mode & KBContactsSelectionModeMessages) {
            if (contact.phonesWithLabels.count > 0 && searchedContact.phonesWithLabels.count > 0)
            {
                ...
                //Added by Palash
                for (int i=0; i<contact.phonesWithLabels.count; i++)
                {
                    NSString *iPostionContactPhone = ((APPhoneWithLabel*) contact.phonesWithLabels[i]).phone;
                    
                    BOOL phonesEqual = [iPostionContactPhone isEqualToString:searchedContactPhone];
    ...
    for (APContact *contact in _contacts) {
        ...
        if (contact.phonesWithLabels.count>1)
        {
            ...
            for (int j=0; j<contact.phonesWithLabels.count; j++)
            {
                ...
                [contactsForFirstLetter addObject:tempContact];
            }
        }
...
- (void)selectAll
{
    [_selectedContactsNumbers removeAllObjects];
...
- (NSArray*)selectedContacts
{
    ...
    [_unmodifiedContacts enumerateObjectsUsingBlock:^(APContact *contact, NSUInteger idx, BOOL *stop) {
        if ([_selectedContactsNumbers containsObject:((APPhoneWithLabel*)contact.phonesWithLabels[0]).phone]) {
...
{
    //NSLog(@"Section Title: %@",_sectionIndexTitles[section]);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (contact) {
        ...
        cell.buttonSelection.selected = [_selectedContactsNumbers containsObject:((APPhoneWithLabel*)contact.phonesWithLabels[0]).phone];
        
        
       //Disable nonvalidation number
        /*BOOL enabled = YES;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    APContact *contact = [self contactAtIndexPath:indexPath];
    if (contact) {
        BOOL selected = [_selectedContactsNumbers containsObject:((APPhoneWithLabel*)contact.phonesWithLabels[0]).phone];
        if (selected) {
            [_selectedContactsNumbers removeObject:((APPhoneWithLabel*)contact.phonesWithLabels[0]).phone];
            if ([_delegate respondsToSelector:@selector(dataSource:didRemoveContact:)]) {
                [_delegate dataSource:self didRemoveContact:contact];
            }
        } else {
            [_selectedContactsNumbers addObject:((APPhoneWithLabel*)contact.phonesWithLabels[0]).phone];
...
- (APContact*)contactAtIndexPath:(NSIndexPath*)indexPath
{
    if (indexPath.section < _contactsGroupedInSections.count) {
        ...
        {
            //NSLog(@"IndexPath ROw: %d and contactsInSection: %@",indexPath.row,contactsInSection[indexPath.row]);
